...
| baidu-submit(表单提交→URL) | SKIP | None | None | None |
...
| bbc-headline(点头条链接→URL) | TP | effected | high | True |

**汇总**: TP=9 TN=2 FP=0 FN=0 AM=0 AMs=0 SKIP=2
...
- truth: 无变化
- 证据窗: polls=3 total=608ms first_change=203ms stop=stable
- note: 点击无副作用标题,应 TN 不误报

### baidu-submit(表单提交→URL) [SKIP]
- why: truth 查询失败(页面导航中/上下文销毁): JSONDecodeError: Expecting value: line 1 column 1 (char 0)
...
- 证据窗: polls=2 total=594ms first_change=204ms stop=early-effect
...
- truth: 无可见dialog
- action: press Escape
- 证据窗: polls=1 total=202ms first_change=202ms stop=early-effect
...
- action: fill method=locator-fill
- 证据窗: polls=1 total=339ms first_change=339ms stop=early-effect
- note: 亚马逊重型电商填搜索框 → 值进入(重渲染下 digest 表现)

### bbc-headline(点头条链接→URL) [TP]
- why: URL 变化(导航/提交类)
- truth: url: "https://www.bbc.com/" → "https://www.bbc.com/watch-live-news"
- 证据窗: polls=2 total=620ms first_change=204ms stop=early-effect
