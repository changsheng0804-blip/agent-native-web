...
**汇总**: TP=8 TN=3 FP=0 FN=0 AM=0 AMs=0 SKIP=2
...
- truth: tab-b选中
- 证据窗: polls=1 total=208ms first_change=208ms stop=early-effect
- note: 无 dialog,SPA 式切换,专测是否误判 effected(FP)或漏判

### negative-heading(负例) [TN]
- why: 目标区域无变化(点击可能未生效,或效果发生在远处)
- truth: 无变化
- 证据窗: polls=3 total=614ms first_change=202ms stop=stable
...
### wiki-link(常规链接跳转) [TP]
- why: 页面整体发生导航: URL 从 https://en.wikipedia.org/wiki/Python_(programming_language) 变为 https://en.wikipedia.org/wiki/Guido_van_Rossum
- truth: url: "https://en.wikipedia.org/wiki/Python_(programming_language) → "https://en.wikipedia.org/wiki/Guido_van_Rossum"
- 证据窗: polls=2 total=691ms first_change=215ms stop=early-effect
...
- 证据窗: polls=3 total=627ms first_change=211ms stop=stable
...
- 证据窗: polls=3 total=650ms first_change=211ms stop=stable
