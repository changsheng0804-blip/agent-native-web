...
6. **世界号 + 稳定指纹(2026-08-31)**:两个跨世界正确性/认路设计——
   ...
   - **真站验证与"语义 id 优先"改进(2026-08-31)**:`test_fingerprint_real.py` 六真站(维基/百度/亚马逊/BBC/GitHub/GF)两轮验证。发现随机 id(GF 的 `vfppkd`、React 的 `__react-xxx`)每次渲染会变,污染指纹跨会话稳定性 → 新增 `isSemanticId` 判定(id 含 `-`/`_` 分隔符或单字含元音=语义 id 才进指纹;纯数字/全辅音/`__` 开头=随机 id 丢弃),祖先路径 3→5 层降撞车。**改进后亚马逊 81→86%、GitHub 81→88%、维基 90→92%**,核心控件命中率 83%(可用)。**结论:该记住的(登录/搜索/导航/表单等有语义命名的操作目标)都记住了(85~92%),记不住的(无命名重复内容如新闻卡片)本不该靠指纹——其内容/文本本身就是天然 ID,需要时用 `world_entities(text=...)` 关联即可。分层:操作目标用指纹认,内容用文本找。**
